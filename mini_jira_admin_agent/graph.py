...
from .tools import add_user_tool, create_ticket_tool, view_ticket_tool, update_status_tool, list_tickets_tool, reset_database_tool, delete_user_tool, delete_ticket_tool
...
def _tool_exec(tool, mapping):
    def run(state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            out = tool.invoke(tool_args)  # tools return a string
...
def build_app():
    ...
    # Terminate nodes
    for node in ["add_user", "create_ticket", "view_ticket", "update_status", "list_tickets", "delete_user", "delete_ticket", "reset_database", "unsupported", "clarify"]:
